...
impl<T: Allocator> FrameAllocator<T> {
    ...
    pub fn alloc(&mut self) -> MemoryResult<FrameTracker> {
        self.allocator
            ...
            .ok_or("no available frame to allocate")    // transform Option<usize> to Result<usize, &str>
            .map(|offset| FrameTracker(self.start_ppn + offset))    // transform Result<usize, &str> to Result<FrameTracker, &str>
    }
